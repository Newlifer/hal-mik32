//! HAL for the 32-bit timer with capture/compare channels (`Timer32_1`).
...
use mik32_pac::Timer32_1 as Timer32Peripheral;
...
/// Timer32_1 basic counter.
///
/// Capture/compare/PWM channel support will be added separately.
...
impl Timer32_1 {
    ...
    pub fn set_clock_source(&mut self, source: ClockSource) {
        RCC::configure_timer32_1_clock(source);
    ...
    pub fn configure_pwm(&mut self, channel: Channel, compare: u32, polarity: PwmPolarity) {
        match channel {
            Channel::Channel0 => {
                self.peripheral.ch1_cntr().write(|w| {
                    let w = w.mode().pwm();
                    match polarity {
            ...
            Channel::Channel1 => {
                self.peripheral.ch2_cntr().write(|w| {
                    let w = w.mode().pwm();
                    match polarity {
                        PwmPolarity::Direct => w.pwm_inv().direct(),
                        PwmPolarity::Inverted => w.pwm_inv().inverted(),
            ...
            Channel::Channel2 => {
                self.peripheral.ch3_cntr().write(|w| {
                    let w = w.mode().pwm();
                    match polarity {
                        PwmPolarity::Direct => w.pwm_inv().direct(),
                        PwmPolarity::Inverted => w.pwm_inv().inverted(),
            ...
            Channel::Channel3 => {
                self.peripheral.ch4_cntr().write(|w| {
                    let w = w.mode().pwm();
                    match polarity {
                        PwmPolarity::Direct => w.pwm_inv().direct(),
                        PwmPolarity::Inverted => w.pwm_inv().inverted(),
    ...
    pub fn set_compare(&mut self, channel: Channel, compare: u32) {
        match channel {
            Channel::Channel0 => self
                .peripheral
                .ch1_ocr()
                .write(|w| unsafe { w.ocr().bits(compare) }),
            Channel::Channel1 => self
                .peripheral
                .ch2_ocr()
                .write(|w| unsafe { w.ocr().bits(compare) }),
            Channel::Channel2 => self
                .peripheral
                .ch3_ocr()
                .write(|w| unsafe { w.ocr().bits(compare) }),
            Channel::Channel3 => self
                .peripheral
                .ch4_ocr()
                .write(|w| unsafe { w.ocr().bits(compare) }),
        };
